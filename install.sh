...
chmod +x "$DOTFILES_DIR/install.sh" "$DOTFILES_DIR/reset-arch.sh" 2>/dev/null || true
if [ -d "$DOTFILES_DIR/scripts/.local/bin" ]; then
    chmod +x "$DOTFILES_DIR/scripts/.local/bin/"* 2>/dev/null || true
fi

# 4. Stow packages into $HOME if present
...
# 6. Reload Hyprland configuration if running
